...
test("database bootstrap is open and reports a missing database", async ({
  request,
}) => {
...
test("agent heartbeat health reports persistent state truthfully", async ({
  request,
}) => {
...
test("production readiness, performance, and promotion gates are visible and fail-closed", async ({
  page,
  request,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("heading", { name: "Shadow Performance" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Shadow Promotion Gate" }),
  ).toBeVisible();

  await page.getByRole("button", { name: "Models", exact: true }).click();
  await expect(
    page.getByRole("heading", { name: "Mercury Intelligence Lab" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Market Feed Fabric" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Historical Replay" }),
  ).toBeVisible();
...
test("validation universe seeding is open and requires Postgres", async ({
  request,
}) => {
  const response = await request.post("/api/admin/seed-validation");
...
test("one-shot shadow activation is open and reports missing persistence", async ({
  request,
}) => {
  const response = await request.post("/api/activation/launch");
  expect(response.status()).toBe(503);
  const json = await response.json();
  expect(json.ok).toBe(false);
...
test("shadow performance remains explicit when persistence is unavailable", async ({
  request,
}) => {
...
test("market provider fabric exposes failover state and open pulling", async ({
  request,
}) => {
...
test("historical replay is visible, open, and explicit without persistence", async ({
  request,
}) => {
  ...
  const response = await request.post("/api/research/history", {
    data: { provider: "auto" },
  });
  expect(response.status()).toBe(503);
  const json = await response.json();
...
test("institutional research APIs are explicit and fail closed without persistence", async ({
  request,
}) => {
  ...
  expect(
    killJson.switches.some(
      (item: { key: string; tripped: boolean }) =>
        item.key === "database" && item.tripped,
    ),
  ).toBeTruthy();
  ...
  const twins = await request.get(
    "/api/research/twins?opportunityId=shadow-test",
  );
  expect(twins.ok()).toBeTruthy();
  expect((await twins.json()).available).toBe(false);
});

test("advanced write paths are open and report missing persistence", async ({
  request,
}) => {
  for (const path of [
