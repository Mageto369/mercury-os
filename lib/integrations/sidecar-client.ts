...
function validateBaseUrl(baseUrl: string) {
  try {
    ...
    if (process.env.NODE_ENV === 'production' && url.protocol !== 'https:') {
      return { ok: false as const, reason: 'sidecar_https_required' };
    }
    if (url.username || url.password) return { ok: false as const, reason: 'sidecar_url_credentials_not_allowed' };
    return { ok: true as const, url };
  } catch {
    return { ok: false as const, reason: 'sidecar_url_invalid' };
  }
}
...
export async function callSidecar<T>(baseUrl:string|undefined, path:string, init?:RequestInit):Promise<SidecarResult<T>> {
  ...
  const token = process.env.MERCURY_SIDECAR_TOKEN;
  const headers = new Headers(init?.headers);
  headers.set('accept', 'application/json');
  if (token) headers.set('authorization', `Bearer ${token}`);

  try {
    const response=await fetch(`${validated.url.toString().replace(/\/$/,'')}${path}`, {
      ...init,
      headers,
    ...
    try {
      return {ok:true, data:JSON.parse(text) as T, latencyMs:Date.now()-started};
    } catch {
      return {ok:false, reason:'sidecar_invalid_json', latencyMs:Date.now()-started};
    }
  } catch(error) {
