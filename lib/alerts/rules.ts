...
export function conditionsMet(
  ...
  for (const [key, expected] of entries) {
    ...
    if (bound) {
      ...
      if (bound.kind === "min" && actual < expected)
        return { met: false, reason: `below_${key}` };
      if (bound.kind === "max" && actual > expected)
        return { met: false, reason: `above_${key}` };
      continue;
    }
    if (!(key in data))
      return { met: false, reason: `condition_field_missing:${key}` };
    if (data[key] !== expected)
      return { met: false, reason: `condition_mismatch:${key}` };
  }
  return { met: true };
}
...
function boundFromKey(
  key: string,
): { kind: "min" | "max"; field: string } | null {
  const match = BOUND_KEY.exec(key);
  if (!match) return null;
  const [, kind, rest] = match;
  return {
    kind: kind as "min" | "max",
    field: rest.charAt(0).toLowerCase() + rest.slice(1),
  };
}
...
export function matchRule(
  ...
  if (rule.category !== event.category)
    return { matched: false, rule, reason: "category_mismatch" };
  ...
  if (!conditions.met)
    return { matched: false, rule, reason: conditions.reason };
...
export function validEmailAddress(value: string | null | undefined) {
  ...
  return /^[^\s@,;<>"]+@[^\s@,;<>"]+\.[A-Za-z]{2,}$/.test(address)
    ? address
    : null;
}
...
export function resolveChannel(
  ...
  if (channel === "email") {
    ...
    if (!apiKey)
      return { kind: "unavailable", reason: "email_api_key_not_configured" };
    const from = validEmailAddress(env.emailFrom);
    if (!from)
      return { kind: "unavailable", reason: "email_sender_not_configured" };
    const to = validEmailAddress(rule.destination);
    if (!to)
      return { kind: "unavailable", reason: "email_recipient_not_configured" };
    const url = String(
      env.emailApiUrl ?? "https://api.resend.com/emails",
    ).trim();
...
export function cooldownActive(
  ...
  const last =
    lastDeliveredAt instanceof Date
      ? lastDeliveredAt
      : new Date(lastDeliveredAt);
