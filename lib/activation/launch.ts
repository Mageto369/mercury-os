...
const activationJobs = [
  'market-regime','liquidity-pulse','social-radar','sec-filings','finra-actions','share-structure','risk-gateway','gem-discovery','model-learning',
] as const;
...
export async function runShadowActivation() {
  ...
  if (!seed.ok) return { ok:false as const, phase:'seed-validation' as const, bootstrap, intelligenceLab, historicalReplay, openDataMesh, modelBaseline, seed, mode:'shadow' as const, capitalExecutionEnabled:false as const, startedAt:startedAt.toISOString(), finishedAt:new Date().toISOString() };
  ...
  return {
    ok:true as const, phase:degraded ? 'shadow-activated-degraded' as const : 'shadow-activated' as const,
    mode:'shadow' as const, capitalExecutionEnabled:false as const,
    bootstrap, intelligenceLab, historicalReplay, openDataMesh, modelBaseline, seed, outcomeMaturation,
    supervisor:{ supervisor:supervisor.supervisor, dueJobs:supervisor.dueJobs, completed:supervisor.completed, degraded:supervisor.degraded, skipped:supervisor.skipped, assignments:supervisor.assignments },
    readiness, performance, fleet:{persistent:fleet.persistent, observedAgents:fleet.heartbeats.length},
