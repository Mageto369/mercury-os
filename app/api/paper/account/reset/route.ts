...
export async function POST(request:Request){
 ...
 try{
  const result=await sql.begin(async tx=>{
   ...
   const resting=await tx`select id from paper_orders where status in('open','pending','partially_filled') for update`;
   ...
   return {status:200,body:{ok:true,status:'reset',startingCapital:Number(account.starting_capital),restingOrdersCancelled:resting.length,historyPreserved:true,capitalExecutionEnabled:false,brokerConnected:false}};
  });
