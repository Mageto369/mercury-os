...
export async function DELETE(
  ...
  try {
    const result = await sql.begin(async (tx) => {
      const [order] =
        await tx`select id,status,filled_qty from paper_orders where id=${id} for update`;
      if (!order)
        return {
          status: 404,
          body: { ok: false, error: "paper_order_not_found" },
        };
      if (
        !["open", "pending", "partially_filled"].includes(String(order.status))
      )
        return {
          status: 409,
          body: {
            ok: false,
            error: "paper_order_not_cancellable",
            status: order.status,
          },
        };
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "paper_order_cancel_failed",
