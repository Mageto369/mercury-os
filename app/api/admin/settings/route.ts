...
import { encryptSecret, vaultConfigured } from '@/lib/admin/vault';
...
const integrationSchema=z.object({
  id:z.string().min(1), enabled:z.boolean(), baseUrl:z.string().url().or(z.literal('')).optional(), model:z.string().max(120).optional(),
  settings:z.record(z.string(),z.unknown()).optional(), secret:z.string().min(1).max(10000).optional(), secretName:z.string().max(80).default('api_key')
});
...
export async function GET(request:Request){
  ...
  const integrations=await sql<any[]>`select c.*, exists(select 1 from integration_secrets s where s.integration_id=c.id) as secret_configured, (select masked_hint from integration_secrets s where s.integration_id=c.id order by s.updated_at desc limit 1) as masked_hint from integration_configs c order by category,display_name`;
  const ingestion=await sql<any[]>`select * from ingestion_settings order by display_name`;
  const monitoring=await sql<any[]>`select * from monitoring_checks order by category,display_name`;
  const audit=await sql<any[]>`select action,target_type,target_ref,outcome,metadata,created_at from admin_audit_log order by created_at desc limit 50`;
  return NextResponse.json({ok:true,vaultConfigured:vaultConfigured(),catalog:integrationCatalog,ingestionCatalog,integrations,ingestion,monitoring,audit,capitalExecutionEnabled:false});
}
