...
export interface SupervisorRunResult {
  ...
  skipped: number;
  escalations: string[];
  controls: { governance: GovernanceResult; dataQuality: DataQualityResult };
}
...
function assignmentFromResult(result: AutonomousJobResult, persisted: boolean): AgentAssignmentResult {
  ...
  return {
    agentId: agent?.id ?? 'mercury-supervisor', agentName: agent?.name ?? 'Mercury Supervisor', job: result.name,
    status: result.status, actionCount: result.actionCount, message: result.message, persisted,
  };
}

async function heartbeat(assignment: AgentAssignmentResult) {
  return recordAgentHeartbeat({
    agentId: assignment.agentId,
    status: assignment.status,
    mission: assignment.job,
    details: { actionCount: assignment.actionCount, message: assignment.message, auditPersisted: assignment.persisted },
  });
}
...
async function runOpportunityDirector(): Promise<AgentAssignmentResult> {
  ...
  try {
    ...
    return {
      agentId: agent.id, agentName: agent.name, job: 'opportunity-director',
      status: result.generated.length ? 'completed' : 'degraded', actionCount: result.generated.length,
    ...
    return {
      agentId: agent.id, agentName: agent.name, job: 'opportunity-director', status: 'degraded', actionCount: 0,
      message: `Vector failed safely: ${error instanceof Error ? error.message : 'unknown opportunity workflow error'}.`, persisted: false,
    };
  }
}

export async function runSupervisor(date = new Date(), requestedJobs?: IntelligenceJobName[], trigger: 'cron' | 'manual' = 'cron'): Promise<SupervisorRunResult> {
  ...
  const selected = requestedJobs?.length ? intelligenceJobs.filter((job) => requestedJobs.includes(job.name)) : jobsDueAt(date);
  const assignments: AgentAssignmentResult[] = [];
  const escalations: string[] = [];

  await recordAgentHeartbeat({
    agentId: 'governance-agent',
    status: governance.status === 'healthy' ? 'completed' : 'degraded',
    mission: 'authority-preflight',
    details: { violations: governance.authorityViolations, guardrailReasons: governance.guardrailReasons },
  });
  await recordAgentHeartbeat({
  ...
  if (supervisorHalted) {
    ...
    for (const job of sortJobs(selected)) {
      ...
      const assignment: AgentAssignmentResult = {
        agentId: agent?.id ?? 'mercury-supervisor', agentName: agent?.name ?? 'Mercury Supervisor', job: job.name,
        status: 'skipped', actionCount: 0, message: `Supervisor halt: ${reasons.join(', ')}.`, persisted: false,
      };
      assignments.push(assignment);
      await heartbeat(assignment);
    }
  } else {
    for (const job of sortJobs(selected)) {
      const result = await executeAutonomousJob(job);
      const audit = await persistAutonomousResult(result, trigger);
      const assignment = assignmentFromResult(result, audit.persisted);
      assignments.push(assignment);
      await heartbeat(assignment);
  ...
  const summaryStatus: AgentAssignmentResult['status'] = escalations.length ? 'degraded' : 'completed';
  await recordAgentHeartbeat({
  ...
  return {
    ...
    startedAt: startedAt.toISOString(), completedAt: new Date().toISOString(), dueJobs: selected.length, assignments,
    ...
    escalations, controls: { governance, dataQuality },
  };
}
