...
export async function pullAndPersistMarketData(maxSymbolsOverride?:number){
 ...
 const universe=await sql`SELECT id, symbol FROM securities WHERE active = true AND id NOT LIKE 'validation:%' ORDER BY symbol LIMIT ${maxSymbols}`;const symbols=universe.map(row=>String(row.symbol));if(!symbols.length)return{ok:false as const,reason:'universe_empty' as const,attempts:[] as MarketProviderPullResult[]};
 ...
 for(const provider of selectedProviders()){if(!provider.configured())continue;const result=await provider.pull(symbols);attempts.push(result);if(result.ok&&result.snapshots.length){winner=result;break}}
