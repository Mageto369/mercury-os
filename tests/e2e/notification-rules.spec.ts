...
test.describe('conditions', () => {
  ...
  test('min and max bounds are enforced against the payload', () => {
    expect(conditionsMet({ minConfidence: 80 }, { confidence: 85 }).met).toBe(true);
    expect(conditionsMet({ minConfidence: 80 }, { confidence: 80 }).met).toBe(true);
    expect(conditionsMet({ minConfidence: 80 }, { confidence: 79 }).met).toBe(false);
    expect(conditionsMet({ maxSpreadBps: 500 }, { spreadBps: 400 }).met).toBe(true);
    expect(conditionsMet({ maxSpreadBps: 500 }, { spreadBps: 900 }).met).toBe(false);
  });
  ...
  test('a condition on a field the payload lacks fails closed', () => {
    ...
    expect(result.met === false && result.reason).toContain('condition_field_missing');
  });

  test('non-numeric payload values do not satisfy a numeric bound', () => {
    expect(conditionsMet({ minConfidence: 80 }, { confidence: 'high' }).met).toBe(false);
    expect(conditionsMet({ minConfidence: 80 }, { confidence: Number.NaN }).met).toBe(false);
  });

  test('non-bound keys compare by equality', () => {
    expect(conditionsMet({ action: 'buy' }, { action: 'buy' }).met).toBe(true);
    expect(conditionsMet({ action: 'buy' }, { action: 'sell' }).met).toBe(false);
  });
});
...
test.describe('rule matching', () => {
  ...
  test('category must match the event', () => {
    const result = matchRule(rule({ category: 'paper' }), event({ category: 'operations' }));
    expect(result.matched === false && result.reason).toBe('category_mismatch');
  });

  test('events below the severity floor are not delivered', () => {
    const result = matchRule(rule({ minimum_severity: 'critical' }), event({ severity: 'high' }));
    expect(result.matched === false && result.reason).toBe('below_minimum_severity');
  });
  ...
  test('conditions are evaluated as part of matching', () => {
    ...
    expect(matchRule(configured, event({ payload: { confidence: 90 } })).matched).toBe(true);
    expect(matchRule(configured, event({ payload: { confidence: 10 } })).matched).toBe(false);
  });
});
...
test.describe('channel resolution fails closed', () => {
  ...
  test('webhook without any configured URL is unavailable', () => {
    const resolved = resolveChannel(rule({ channel: 'webhook' }), { alertWebhookUrl: null });
    expect(resolved.kind === 'unavailable' && resolved.reason).toBe('webhook_url_not_configured');
  });

  test('webhook falls back to the environment URL when the rule has no destination', () => {
    const resolved = resolveChannel(rule({ channel: 'webhook' }), { alertWebhookUrl: 'https://hooks.example.com/a' });
    expect(resolved.kind).toBe('http');
    expect(resolved.kind === 'http' && resolved.url).toBe('https://hooks.example.com/a');
  });

  test('a rule destination overrides the environment webhook', () => {
    const resolved = resolveChannel(
      rule({ channel: 'webhook', destination: 'https://rule.example.com/hook' }),
      { alertWebhookUrl: 'https://env.example.com/hook' },
    );
    expect(resolved.kind === 'http' && resolved.url).toContain('rule.example.com');
  });

  test('slack requires its own destination and never borrows the generic webhook', () => {
    const resolved = resolveChannel(rule({ channel: 'slack' }), { alertWebhookUrl: 'https://env.example.com/hook' });
    expect(resolved.kind === 'unavailable' && resolved.reason).toBe('slack_webhook_url_not_configured');
  });
});
...
test.describe('destination validation', () => {
  test('rejects credentials embedded in the URL', () => {
    ...
    expect(result.ok === false && result.reason).toBe('destination_url_credentials_not_allowed');
  });
  ...
  test('accepts a public https endpoint', () => {
    expect(validateDestination('https://hooks.example.com/services/x').ok).toBe(true);
  });
});
