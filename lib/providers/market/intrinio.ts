...
export const intrinioMarketProvider: MarketProvider = {
  ...
  async pull(symbols): Promise<MarketProviderPullResult> {
    ...
    const concurrency = Math.max(1, Math.min(15, Number(process.env.MARKET_PULL_CONCURRENCY ?? 6)));
      ...
      const results = await Promise.all(batch.map(async (symbol) => {
        try {
          ...
          return {
            ...
            observedAt: data.last_time ? new Date(data.last_time) : new Date(),
            source: 'intrinio' as const,
            providerPayload: {},
          } satisfies NormalizedMarketSnapshot;
        } catch (error) {
          errors.push({ symbol, message: error instanceof Error ? error.message : 'intrinio_pull_failed' });
          return null;
        }
      }));
