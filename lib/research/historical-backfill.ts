...
type Provider = 'massive' | 'intrinio';
...
async function pullMassive(days: string[], tracked: Set<string>) {
  ...
  for (const day of days) {
    try {
      ...
      url.searchParams.set('include_otc', 'true');
      ...
      for (const row of data.results ?? []) {
        ...
        bars.push({
          symbol,
          time: new Date(Number(row.t ?? Date.parse(`${day}T00:00:00Z`))),
          open: Number(row.o ?? 0) || undefined,
          high: Number(row.h ?? 0) || undefined,
          low: Number(row.l ?? 0) || undefined,
          close,
          volume: Number(row.v ?? 0) || undefined,
          vwap: Number(row.vw ?? 0) || undefined,
          transactions: Number(row.n ?? 0) || undefined,
          source: 'massive',
        });
      }
    } catch (error) {
      errors.push(`${day}:${error instanceof Error ? error.message : 'massive_backfill_failed'}`);
    }
  }
  return { bars, errors };
}
  ...
  for (let i = 0; i < symbols.length; i += concurrency) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        ...
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 12_000);
        const response = await fetch(url, { cache: 'no-store', signal: controller.signal, headers: { Authorization: `Bearer ${key}` } });
        clearTimeout(timeout);
        if (!response.ok) throw new Error(`http_${response.status}`);
        const data = await response.json() as { stock_prices?: Array<Record<string, unknown>> };
        for (const row of data.stock_prices ?? []) {
          const close = Number(row.close ?? row.adj_close ?? 0);
          const date = String(row.date ?? '');
          if (!date || !Number.isFinite(close) || close <= 0) continue;
          bars.push({
            symbol,
            time: new Date(`${date}T00:00:00.000Z`),
            open: Number(row.open ?? row.adj_open ?? 0) || undefined,
            high: Number(row.high ?? row.adj_high ?? 0) || undefined,
            low: Number(row.low ?? row.adj_low ?? 0) || undefined,
            close,
            volume: Number(row.volume ?? row.adj_volume ?? 0) || undefined,
            source: 'intrinio',
          });
        }
      } catch (error) {
        errors.push(`${symbol}:${error instanceof Error ? error.message : 'intrinio_backfill_failed'}`);
      }
    }));
  }
  return { bars, errors };
...
export async function backfillHistoricalMarket(input?: { startDate?: string; endDate?: string; provider?: Provider | 'auto' }) {
  ...
  const maxDays = Math.max(1, Math.min(3650, Number(process.env.HISTORICAL_BACKFILL_MAX_DAYS ?? 730)));
  const days = dateRange(startDate, endDate);
  if (days.length > maxDays) return { ok: false as const, reason: 'date_range_too_large' as const, maxDays };

  const universe = await sql`SELECT id, symbol FROM securities WHERE active = true ORDER BY symbol`;
  const ids = new Map(universe.map((row) => [String(row.symbol).toUpperCase(), String(row.id)]));
  const symbols = [...ids.keys()];
  ...
  const provider: Provider = configured === 'intrinio' ? 'intrinio' : configured === 'massive' ? 'massive' : (process.env.MASSIVE_API_KEY || process.env.MARKET_DATA_API_KEY ? 'massive' : 'intrinio');
  ...
  const pulled = provider === 'massive'
    ? await pullMassive(days, new Set(symbols))
    : await pullIntrinio(symbols, startDate, endDate);

  let inserted = 0;
  for (const bar of pulled.bars) {
    const securityId = ids.get(bar.symbol);
    if (!securityId) continue;
    const result = await sql`
      INSERT INTO historical_bars (id, security_id, timeframe, bar_time, open, high, low, close, volume, vwap, transactions, adjusted, source, payload)
      VALUES (${randomUUID()}, ${securityId}, '1d', ${bar.time}, ${bar.open ?? null}, ${bar.high ?? null}, ${bar.low ?? null}, ${bar.close}, ${bar.volume ?? null}, ${bar.vwap ?? null}, ${bar.transactions ?? null}, true, ${bar.source}, ${JSON.stringify({ backfillRunId: runId })}::jsonb)
      ON CONFLICT (security_id, timeframe, bar_time, source) DO NOTHING
      RETURNING id
    `;
  ...
  return {
    ...
    barsReceived: pulled.bars.length,
...
export async function getHistoricalBackfillStatus() {
  const sql = getSql();
  if (!sql) return { available: false as const, reason: 'database_not_configured' as const };
  try {
    const [latest] = await sql`SELECT * FROM backfill_runs ORDER BY started_at DESC LIMIT 1`;
    const [coverage] = await sql`SELECT count(*)::int AS bars, count(distinct security_id)::int AS securities, min(bar_time) AS first_bar, max(bar_time) AS last_bar FROM historical_bars`;
    return { available: true as const, mode: 'shadow' as const, capitalExecutionEnabled: false as const, latest: latest ?? null, coverage: coverage ?? null };
  } catch {
    return { available: false as const, reason: 'historical_schema_not_initialized' as const };
  }
}
