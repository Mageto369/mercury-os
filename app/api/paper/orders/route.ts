  ...
  try {
    ...
    const result = await sql.begin(async (tx) => {
      ...
      if (!security)
        return {
          status: 404,
          body: { ok: false, error: "security_not_found" },
        };
      ...
      const sidePrice =
        input.side === "buy"
          ? Number(snapshot.ask ?? snapshot.price)
          : Number(snapshot.bid ?? snapshot.price);
      const referencePrice =
        Number.isFinite(sidePrice) && sidePrice > 0 ? sidePrice : mark;
      ...
      const slippedPrice =
        input.side === "buy"
          ? referencePrice * (1 + slip)
          : referencePrice * (1 - slip);
      const fillPrice = clampSimulatedFillPrice(
        input.orderType,
        input.side,
        slippedPrice,
        requestedPrice,
      ...
      const commissionBps = Math.max(
        0,
        Number(process.env.PAPER_COMMISSION_BPS ?? 0),
      );
      const feeAmount = Number(
        ((notional * commissionBps) / 10_000).toFixed(6),
      );
      ...
      if (input.orderType === "limit") {
        ...
        if (!crosses) {
          ...
          const restingReason =
            input.side === "buy" &&
            currentCash < input.quantity * requestedPrice
      ...
      if (simulation.capacityExceeded)
        rejectReason = "liquidity_capacity_exceeded";
      if (
        input.side === "buy" &&
        currentCash < input.quantity * fillPrice + feeAmount
      )
        rejectReason = "insufficient_virtual_cash";
      if (input.side === "sell" && currentQty < input.quantity)
        rejectReason = "insufficient_virtual_position";
      ...
      if (input.side === "buy") {
        ...
        const newAvg =
          newQty > 0
            ? (currentQty * currentAvg + input.quantity * fillPrice) / newQty
            : 0;
        ...
        const realized = toLedgerAmount(
          (fillPrice - currentAvg) * input.quantity - feeAmount,
        );
        const newQty = currentQty - input.quantity;
      ...
      const [postAccount] =
        await tx`select cash,realized_pnl from paper_accounts where id=${DEFAULT_PAPER_ACCOUNT_ID}`;
      const [marking] =
