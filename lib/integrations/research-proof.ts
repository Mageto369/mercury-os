...
import { resolveIntegrationBaseUrl, resolveIntegrationToken } from '@/lib/admin/integration-runtime';
...
async function runtime() {
  const url = await resolveIntegrationBaseUrl('research-proof-sidecar', ['RESEARCH_PROOF_URL','VECTORBT_URL','QUANTSTATS_URL','BACKTRADER_URL','YFINANCE_SIDECAR_URL']);
  const token = await resolveIntegrationToken('research-proof-sidecar', ['MERCURY_SIDECAR_TOKEN'], 'service_token');
  return { url, token };
}

export async function getResearchProofStatus() {
  const {url,token}=await runtime();
  const result = await callSidecar<HealthResponse>(url, '/health', undefined, token);
  return result.ok ? { available:true as const, configured:true, ...result.data, latencyMs:result.latencyMs }
    : { available:false as const, configured:Boolean(url), reason:result.reason, latencyMs:result.latencyMs, mode:'shadow' as const, capitalExecutionEnabled:false as const };
}
...
export async function runResearchProofCycle() {
  ...
  const {url,token}=await runtime();
  if (!url) return { ok:false as const, reason:'research_proof_sidecar_not_configured' as const, mode:'shadow' as const, capitalExecutionEnabled:false as const };
  const rows = await sql`SELECT oo.evaluated_at,oo.return_60m FROM opportunity_outcomes oo JOIN opportunities o ON o.id=oo.opportunity_id JOIN securities s ON s.id=o.security_id WHERE oo.matured_60m=true AND oo.return_60m IS NOT NULL AND s.id NOT LIKE 'validation:%' ORDER BY oo.evaluated_at ASC LIMIT 1500`;
  ...
  const timestamps=rows.map(r=>new Date(String(r.evaluated_at)).toISOString()); const returns=rows.map(r=>Number(r.return_60m)/100); const prices:number[]=[100]; for(const value of returns.slice(1))prices.push(prices[prices.length-1]*(1+value));
  const quantstats=await callSidecar<QuantStatsResponse>(url,'/quantstats/proof',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({timestamps,returns,scope:'live-60m-outcomes'})},token);
  const entries=prices.map((_,i)=>i>0&&i%5===1); const exits=prices.map((_,i)=>i>1&&i%5===4);
  const vectorbt=await callSidecar<VectorBtResponse>(url,'/vectorbt/experiment',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({timestamps,prices,entries,exits,fees:0.001,slippage:0.001,init_cash:100000})},token);
  ...
  if(leader&&Number(leader.bars)>=40){const bars=await sql`SELECT bar_time,open,high,low,close,volume FROM historical_bars WHERE security_id=${String(leader.security_id)} AND timeframe='1d' AND open IS NOT NULL AND high IS NOT NULL AND low IS NOT NULL AND close IS NOT NULL ORDER BY bar_time ASC LIMIT 500`; backtrader=await callSidecar<BacktraderResponse>(url,'/backtrader/challenger',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({timestamps:bars.map(r=>new Date(String(r.bar_time)).toISOString()),open:bars.map(r=>Number(r.open)),high:bars.map(r=>Number(r.high)),low:bars.map(r=>Number(r.low)),close:bars.map(r=>Number(r.close)),volume:bars.map(r=>Number(r.volume??0)),fast:10,slow:30,commission:0.001,cash:100000})},token);}
